...
Run: `cd /home/user/src/go-enriche && GOWORK=off go test ./extract/ -run TestValidatePhone -v -count=1`
...
```bash
cd /home/user/src/go-enriche && git add extract/validate.go extract/validate_test.go
...
Run: `cd /home/user/src/go-enriche && GOWORK=off go test ./extract/ -run TestValidatePrice -v -count=1`
...
Run: `cd /home/user/src/go-enriche && GOWORK=off go test ./extract/ -run TestValidateAddress -v -count=1`
...
Run: `cd /home/user/src/go-enriche && GOWORK=off go test ./extract/ -run "TestExtractFacts_Rejects|TestExtractFacts_AcceptsValid" -v -count=1`
...
Run: `cd /home/user/src/go-enriche && GOWORK=off go test ./extract/ -v -count=1`
...
```bash
cd /home/user/src/go-enriche && git add extract/facts.go extract/facts_test.go
...
Run: `cd /home/user/src/go-enriche && GOWORK=off go test ./extract/ -run TestExtractSnippetFacts_RejectsGarbagePhone -v -count=1`
...
Run: `cd /home/user/src/go-enriche && GOWORK=off go test ./extract/ -run TestExtractSnippetFacts -v -count=1`
Expected: ALL PASS
...
Run: `cd /home/user/src/go-enriche && GOWORK=off go test ./... -count=1`
Expected: ALL PASS

- [ ] **Step 6: Commit**

```bash
cd /home/user/src/go-enriche && git add extract/facts.go extract/snippet_test.go
...
Run: `cd /home/user/src/go-wp && GOWORK=off go test ./... -count=1`
Expected: ALL PASS

- [ ] **Step 5: Commit and deploy go-wp**

```bash
cd /home/user/src/go-wp && git add go.mod go.sum
git commit -m "deps: update go-enriche to v1.4.0 (fact validation layer)"
git push origin main
cd ~/deploy/server-config && docker compose build --no-cache go-wp && docker compose up -d --no-deps --force-recreate go-wp
```
